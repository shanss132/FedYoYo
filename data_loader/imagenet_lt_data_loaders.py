...
from data_loader.autoaug import ImageNetPolicy
...
class LT_Dataset(Dataset):
    ...
    def __getitem__(self, index):
        ...
        if self.transform is not None:
            sample1 = self.transform[1](sample)
            sample2 = self.transform[2](sample)
...
# class noaug_LT_Dataset(Dataset):
...
class ImageNetLTDataLoader(DataLoader):
    ...
    def __init__(self, data_dir, batch_size, shuffle=True, num_workers=1, training=True,  balanced=False, retain_epoch_size=True, 
                 train_txt="./data/ImageNet_LT/ImageNet_LT_train.txt", 
                 val_txt="./data/ImageNet_LT/ImageNet_LT_val.txt", 
                 test_txt="./data/ImageNet_LT/ImageNet_LT_test.txt"):
        randaug_n=2 
        randaug_m=10
        ...
        train_trsfm_augmentation_randncls = transforms.Compose(augmentation_randncls)
        if training:          
            dataset = LT_Dataset(data_dir,  train_txt, [test_trsfm, train_trsfm, train_trsfm_augmentation_randncls])
            val_dataset = test_LT_Dataset(data_dir, val_txt, test_trsfm)
        else: # test
            dataset = test_LT_Dataset(data_dir, test_txt, test_trsfm)
            val_dataset = None
